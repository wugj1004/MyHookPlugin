...
public class PluginHelper {
    ...
    private static final String FIX_DEX_PATH = "odex";
    ...
    private static void loadPluginClass(Context context, ClassLoader hostClassLoader) throws Exception {
        ...
        // Step2.1. 获取optimized否则无法构建DexClassLoader
        File optimizedFile = context.getDir(FIX_DEX_PATH, Context.MODE_PRIVATE);

        // Step2.2. 创建插件的DexClassLoader
        DexClassLoader pluginClassLoader = new DexClassLoader(pluginFile.getAbsolutePath(), optimizedFile.getAbsolutePath(),
    ...
    public static void initPluginResource(Context context) throws Exception {
        ...
        sPluginResources = new Resources(assetManager, context.getResources().getDisplayMetrics(),
